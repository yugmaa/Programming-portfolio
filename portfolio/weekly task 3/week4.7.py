"""program  reads 6 temperatures and displays the maximum, minimum, and mean of the values."""

import statistics

def temperatures(): #6 temperature input
    user_temperature=[]
    for i in range(6):
       user_input = input(f"Enter temperature {i+1} in celsius (example: 20C): ")
       user_temperature.append(celsius_to_fahrenhiet(user_input))
...
read_temperature=temperatures()
maximum_value= maximum(read_temperature)
minimum_value= minimum(read_temperature)
mean_output= mean(read_temperature)
print(f"the maximum temperature is {maximum_value}C.")
print(f"the minimum temperature is {minimum_value}C.")
